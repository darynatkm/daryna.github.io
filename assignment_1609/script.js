...
const watchList = [
  ...
  function getRating(watchList) {
    ...
    const newList = watchList.filter(movie => movie.Director == 'Christopher Nolan').map(movie => ({rating: parseFloat(movie.imdbRating)}))
    averageRating = (newList.reduce((sum, movie) => sum + movie.rating, 0))/newList.length
    // Only change code above this line
  ...
  const squareList = arr => {
    // Only change code below this line
    const newArr = arr.filter(num => num > 0 && Number.isInteger(num)).map(num => num * num)
    return newArr;
    // Only change code above this line
  };
...
function urlSlug(title) {
    ...
    let arr = title.trim().split(/[^a-zA-Z]+/);
